        ...
        def _inner(*args, **kwargs):
            for i in xrange(times):
                ...
                try:
                    ...
                    pass
            raise Exception('run %s times > %d' % (func.__name__, times)) #触发异常
...
class MyOrder(object):
    ...
    def queryOrderWaitTime(self):
        ...
        int(time.time()), self.repeatSubmitToken)
        r = self.s.get(url, timeout=3, verify=False)
        if r.status_code != 200:
        ...
        if resp['status'] and resp['data'].get('orderId', None):
            ...
            print u"等待waitTime=%s,waitCount=%s" % (
            resp['data'].get('waitTime', 'None'), resp['data'].get('waitCount', 'None'))
            time.sleep(0.5)
